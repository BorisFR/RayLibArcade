...
PNG_STATIC uint8_t PNGMakeMask(PNGDRAW *pDraw, uint8_t *pMask, uint8_t ucThreshold)
{
    ...
    case PNG_PIXEL_TRUECOLOR_ALPHA: // truecolor + alpha
        ...
        { // groups of 8 pixels in each byte of mask
            c = 0;
            for (i = 0; i < 8; i++)
            {
                c <<= 1;
...
PNG_STATIC void PNGRGB565(PNGDRAW *pDraw, uint16_t *pPixels, int iEndiannes, uint32_t u32Bkgd, int iHasAlpha)
{
    ...
    case PNG_PIXEL_INDEXED: // palette color (can be 1/2/4 or 8 bits per pixel)
        if (pDraw->pFastPalette && !pDraw->iHasAlpha)
        { // faster RGB565 palette exists
            switch (pDraw->iBpp)
            {
            case 8:
                for (x = 0; x < pDraw->iWidth; x++)
        ...
        case 8: // 8-bit palette also supports palette alpha
            if (pDraw->iHasAlpha)
            { // use the alpha to modify the palette
                if (u32Bkgd != 0xffffffff)
...
PNG_STATIC int PNGParseInfo(PNGIMAGE *pPage)
{
    ...
    if (iBytesRead < 33)
    { // a PNG file this tiny? probably bad
        pPage->iError = PNG_INVALID_FILE;
        return pPage->iError;
    }

    if (MOTOLONG(s) != (int32_t)0x89504e47)
    ...
    if (MOTOLONG(&s[12]) == 0x49484452 /*'IHDR'*/)
    {
        ...
        if (pPage->iInterlaced || pPage->ucBpp > 8)
        { // 16-bit pixels are not supported (yet)
...
{
    int64_t iBytesRead, iOffset;
    int64_t y, iFileOffset;
    int64_t err, iLen = 0;
    int bDone;
    int iMarker = 0;
    ...
    y = (uint32_t)(intptr_t)&pPage->ucPixels[0];
            ...
            while (iLen)
            {
                ...
                while (err == Z_OK)
                {
                    ...
                    { // successfully decoded line
                        ...
                        if (pPage->pImage == NULL)
                        { // no image buffer, send it line by line
                            PNGDRAW pngd;
                            pngd.pUser = pUser;
