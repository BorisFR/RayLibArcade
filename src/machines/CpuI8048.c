/*
ORIGIN: https://github.com/harbaum/galagino/blob/main/galagino/i8048.c
...
#define I8048_GET_BIT(val, bit) ((val >> bit)&1)
#define I8048_SET_BIT(val, bit) (val | (1<<bit))
#define I8048_CLR_BIT(val, bit) (val & ~(1<<bit))
#define I8048_SET_BIT_TO(val, bit, state)  ((val & ~(1<<bit)) | ((state?1:0)<<bit))
...
static inline unsigned char readReg(struct i8048_state_S *state, unsigned char reg) {
  unsigned short base = !I8048_GET_BIT(state->PSW, I8048_BS_BIT) ? I8048_REGISTER_BANK_0_BASE : I8048_REGISTER_BANK_1_BASE;
...
static inline void setCarry(struct i8048_state_S *state, boolean carry) {
  state->PSW = I8048_SET_BIT_TO(state->PSW, I8048_CY_BIT, carry);
}
...
static void addToAcc(struct i8048_state_S *state, int value) {
  ...
  state->PSW = I8048_SET_BIT_TO(state->PSW, I8048_AC_BIT, oldLoNibble > loNibble);
}
...
void i8048_step(struct i8048_state_S *state) {
  ...
  case 0x13: // ADDC A, #data
    addToAcc(state, I8048_GET_BIT(state->PSW, I8048_CY_BIT) + fetch(state));
    break;
  ...
  case 0x57: { // DA A
    if ( (state->A & 0x0f) > 9 || I8048_GET_BIT(state->PSW, I8048_AC_BIT)) 
      state->A += 9;
    
    int hiNibble = (state->A & 0xf0) >> 4;
    if (hiNibble > 9 || I8048_GET_BIT(state->PSW, I8048_CY_BIT)) 
      hiNibble += 6;
  ...
  case 0x67: { // RRC A
    ...
    state->A = I8048_SET_BIT_TO(state->A >> 1, 7, I8048_GET_BIT(state->PSW, I8048_CY_BIT));
  ...
  case 0x70: case 0x71:  // ADDC A, @Rr
    addToAcc(state, I8048_GET_BIT(state->PSW, I8048_CY_BIT) + ram_read(state, readReg(state, op & 0x1)));
    break;
  ...
  case 0x77:  // RR A
    ...
    // state->A = I8048_SET_BIT_TO(state->A>>1, 7, state->A&1);
  ...
  case 0x78: case 0x79: case 0x7a: case 0x7b: case 0x7c: case 0x7d: case 0x7e: case 0x7f:
    // ADDC A, Rr
    addToAcc(state, I8048_GET_BIT(state->PSW, I8048_CY_BIT) + readReg(state, op & 0x7));
    break;
  ...
  case 0x85: // CLR F0
    state->PSW = I8048_CLR_BIT(state->PSW, I8048_F0_BIT);
    break;
  ...
  case 0x95:  // CPL F0
    state->PSW = I8048_SET_BIT_TO(state->PSW, I8048_F0_BIT, !I8048_GET_BIT(state->PSW, I8048_F0_BIT));
    break;
  ...
  case 0x97:  // CLR C
    state->PSW = I8048_CLR_BIT(state->PSW, I8048_CY_BIT);
    break;
  ...
  case 0xa7:  // CPL C
    state->PSW = I8048_SET_BIT_TO(state->PSW, I8048_CY_BIT, !I8048_GET_BIT(state->PSW, I8048_CY_BIT));
    break;
  ...
  case 0xb6:  // JF0 addr
    cjump(state, I8048_GET_BIT(state->PSW, I8048_F0_BIT));
    break;
  ...
  case 0xc5:  // SEL RB0
    state->PSW = I8048_CLR_BIT(state->PSW, I8048_BS_BIT);
    break;
  ...
  case 0xd5: // SEL RB1
    state->PSW = I8048_SET_BIT(state->PSW, I8048_BS_BIT);
    break;
  ...
  case 0xe6: // JNC addr
    cjump(state, !I8048_GET_BIT(state->PSW, I8048_CY_BIT));
    break;
  ...
  case 0xf7: { // RLC A
    ...
    state->A = (state->A << 1) & 0xff;
    if(I8048_GET_BIT(state->PSW, I8048_CY_BIT)) state->A |= 1;
